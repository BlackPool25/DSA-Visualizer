...
/**
 * Comprehensive validation for compilation requests.
 *
 * @param code - The code to compile
 * @returns Object with valid flag and optional error message
 */
export function validateCompileRequest(code: string): { valid: boolean; error?: string } {
  // Check code size
  const sizeCheck = validateCodeSize(code);
  if (!sizeCheck.valid) {
    return sizeCheck;
  }

  // Check for dangerous patterns (warning only, Docker provides real security)
  const safetyCheck = scanForDangerousPatterns(code);
  if (!safetyCheck.safe) {
    // Log warning but allow - Docker security will handle it
    console.warn('Potentially dangerous code patterns detected:', safetyCheck.warning);
  }
...
/**
 * Comprehensive validation for run requests.
 *
 * @param binaryId - The binary ID
 * @param stdin - The input to provide
 * @returns Object with valid flag and optional error message
 */
export function validateRunRequest(binaryId: string, stdin: string): { valid: boolean; error?: string } {
  // Validate binaryId format (should be UUID)
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(binaryId)) {
    return { valid: false, error: 'Invalid binary ID format' };
  }

  // Validate input size
  const inputCheck = validateInputSize(stdin);
  if (!inputCheck.valid) {
    return inputCheck;
  }
...
/**
 * Comprehensive validation for trace requests.
 *
 * @param code - The code to trace
 * @param stdin - The input to provide
 * @param maxSteps - Maximum trace steps
 * @returns Object with valid flag and optional error message
 */
export function validateTraceRequest(
  code: string,
  stdin: string,
  maxSteps: number
): { valid: boolean; error?: string } {
  // Validate code
  const codeCheck = validateCompileRequest(code);
  if (!codeCheck.valid) {
    return codeCheck;
  }
  ...
  // Validate maxSteps
  const stepsCheck = validateMaxSteps(maxSteps);
  if (!stepsCheck.valid) {
    return stepsCheck;
  }
